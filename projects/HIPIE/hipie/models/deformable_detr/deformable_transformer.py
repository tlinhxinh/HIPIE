...
class DeformableTransformerVL(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        self.decoder = DeformableTransformerDecoder(decoder_layer, num_decoder_layers, return_intermediate_dec, look_forward_twice, use_checkpoint)
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, mask_on=False, language_dict_features=None, task=None):
        ...
        ref_feat = repeat(ref_feat, 'b c -> b q c', q=two_stage_num_proposals) # (bs, 300, 256)
        ...
        if self.two_stage:
            ...
            topk_class_unact = torch.gather(enc_outputs_class, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, enc_outputs_class.shape[-1]))
            reference_points = topk_coords_unact.detach().sigmoid()
            init_reference_out = reference_points
            pos_trans_out = self.pos_trans_norm(self.pos_trans(self.get_proposal_pos_embed(topk_coords_unact.detach())))
...
class DeformableTransformerDecoder(nn.Module):
    def __init__(self, decoder_layer, num_layers, return_intermediate=False, look_forward_twice=False, use_checkpoint=False):
        super().__init__()
