...
class DeformableTransformerVLDINO(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        if cfg.MODEL.USE_ADDITIONAL_BERT:
            lang_cfg = BertConfig.from_pretrained("HIPIE/projects/HIPIE/bert-base-uncased")
        ...
        # Hack TODO: Add to config
        self.background_proposals = cfg.MODEL.DDETRS.TWO_STAGE_NUM_BG_PROPOSALS
        self.bg_query = self.background_proposals > 0
        if self.bg_query:
            self.tgt_embed_bg = nn.Embedding(self.background_proposals, d_model)
            self.bg_query_refs = nn.Embedding(self.background_proposals, 4)
        if two_stage:
    ...
    attn_masks=None, return_src_info=False,bg_queries_lang=None,enc_mask=False):
        ...
        # BG QUERY
        if self.bg_query:
            bg_query = self.tgt_embed_bg.weight[None].repeat(bs, 1, 1)
            tgt = torch.cat([bg_query,tgt],dim=1)
            #bg_ref_pt = torch.zeros((*bg_query.shape[:-1],4),dtype=bg_query.dtype,device=bg_query.device)
            bg_ref_pt = self.bg_query_refs.weight[None].repeat(bs, 1, 1)
            reference_points = torch.cat([bg_ref_pt, reference_points], dim=1)
        if bg_queries_lang is not None and len(bg_queries_lang) > 0:
            bg_queries_lang = bg_queries_lang.unsqueeze(0).repeat(bs, 1, 1)
            tgt = torch.cat([bg_queries_lang,tgt],dim=1)
            reference_points = torch.cat([torch.zeros((bs,bg_queries_lang.shape[1],4)).float().to(tgt.device), reference_points], dim=1)
        if query_embed[1] is not None:
            reference_points = torch.cat([query_embed[1].sigmoid(), reference_points], 1)
        init_reference_out = reference_points
        # Additonal process
        N_type_query = len(query_embed)
        # BG QUERY
        if N_type_query > 2:
            tgt = torch.cat([query_embed[2],tgt],1)
        ...
        if enc_mask:
            enc_outputs_class = (enc_outputs_class,output_memory) # hack
        if mask_on:
            if return_src_info:
                src_info_dict = {
                    "src": memory.detach(),
